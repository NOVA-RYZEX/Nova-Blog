export default defineAppConfig({
  ui: {
    ...
    button: {
      ...
      slots: {
        base: "rounded-xl duration-200 cursor-pointer",
      },
    },
    badge: {
      defaultVariants: {
    ...
    input: {
      ...
      slots: {
        root: "transition-all rounded-xl",
      },
    },
    avatar: {
      defaultVariants: {
    ...
    card: {
      slots: {
        root: "rounded-2xl border border-gray-200/60 dark:border-gray-800/60 backdrop-blur-md shadow-xs hover:shadow-xl hover:shadow-primary/5 transition-all overflow-hidden",
        body: "flex gap-4 p-6 sm:p-8",
      },
    },
    formField: {
      slots: {
        container: "flex flex-col gap-1.5",
        help: "m-0 font-medium",
        label: "text-sm text-gray-900 dark:text-white",
      },
    },
    link: {
      variants: {
    ...
    // Header specific configurations isolated precisely from other global variables
    header: {
      slots: {
